...
struct SimpleProfiler : public nvinfer1::IProfiler {
    ...
    friend std::ostream &operator<<(std::ostream &out, const SimpleProfiler &value) {
        ...
        for (std::size_t i = 0; i < value.mLayerNames.size(); i++) {
...
// Swaps endianness of an integral type.
    ...
    inline void *safeCudaMalloc(std::size_t memSize) {
        void *deviceMem;
...
//! Return vector of indices that puts magnitudes of sequence in descending order.
    template<class Iter>
    std::vector<std::size_t> argMagnitudeSort(Iter begin, Iter end) {
    ...
    std::vector<std::string> classify(
            const std::vector<std::string> &refVector, const std::vector<T> &output, const std::size_t topK) {
...
// Returns indices of highest K magnitudes in v.
    template<typename T>
    std::vector<std::size_t> topKMagnitudes(const std::vector<T> &v, const std::size_t k) {
    ...
    template<typename T>
    bool readASCIIFile(const std::string &fileName, const std::size_t size, std::vector<T> &out) {
        std::ifstream infile(fileName);
        if (!infile.is_open()) {
...
// cases where scaling factors are asymmetric.
    ...
    // 虽然accumulate是累加的意思，但是第四个参数std::multiplies<int64_t>()表示要进行的操作是 连乘！！
    // 前两个参数表示连续内存的起始位置，左闭右开（即结束位置处的值不取）
