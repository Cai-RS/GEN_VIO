...
__device__ void Selection_sort(float *data, int left, int right) 
{
  ...
  {
    float min_val = data[i*size_pt];
    int min_idx = i;

    // Find the largest value in the range [left, right].
    for (int j = i + 1; j <= right; ++j) 
    {
        float val_j = data[j*size_pt];
    ...
    {
      float temp_value = 0;
      for(int k=0; k<size_pt; k++)
      {
        temp_value = data[min_idx*size_pt+k];
        data[min_idx*size_pt+k] = data[i*size_pt+k];
        data[i*size_pt+k] = temp_value;
      }
    }
  }
}
...
// 基于GPU动态并行的快排
// 将匹配按照ambi从小到大排序
__global__ void simple_quicksort(float *data_ptr, int left, int set_right, int depth)
{
  // 当递归的深度大于设定的MAX_DEPTH或者待排序的数组长度小于设定的阈值，直接调用简单选择排序
  int right = set_right;

  if (depth >= MAX_DEPTH_2 || right - left <= INSERTION_SORT) 
  {
      // 凡是在kernel函数中调用的自定义函数，都是__device__函数，即必须由device调用与执行
      Selection_sort(data_ptr, left, right);
      return;
  }

  size_t size_pt = 2;
  // 找到当前区间的左端和右端bbox中的conf元素所在内存
  float *left_ptr  = data_ptr + left * size_pt;
  float *right_ptr = data_ptr + right * size_pt;
  float pivot = data_ptr[(left + right) / 2 * size_pt];

  // partition
  // partition停止时，左指针left_ptr会比右指针right_ptr大，此时它们各自称为下一个被partition的区间的左和右起始位置
...
{
  // M7W和个 128维的float数组（应该就是一个siftpoint） 作为此线程中的共享数据
  __shared__ float4 buffer1[M7W*NDIM/4]; 
  __shared__ float4 buffer2[M7H*NDIM/4];
  // tx和ty的值都是[0, 15]       
  int tx = threadIdx.x;
  int ty = threadIdx.y;
  // 每个kernel负责与M7W个其他的siftpoint点进行corr？
  int bp1 = M7W*blockIdx.x;
  // j每次+8。为什么x方向会和y方向产生关系？是否应该改为j<M7H?
  for (int j=ty;j<M7W;j+=M7H/M7R) {    
    int p1 = min(bp1 + j, numPts1 - 1);
    // d每次直接+32？？？
    for (int d=tx;d<NDIM/4;d+=M7W)
    // data为128个元素的float数组，用float4型指针指向它，即将其中每4个float元素归为一个float4结构体，共32个struct对象
...
// 只从场景中寻找内点数最多的一个H，即sift特征点最多的一个平面
double FindHomography(SiftData &data, float *homography, int *numMatches, int numLoops, float minScore, float maxAmbiguity, float thresh)
{
  *numMatches = 0;
  ...
  // 每次迭代使用4个int型随机数，这个int型变量是siftpoint的序号，即用四对点来计算H矩阵
  int randSize = 4*sizeof(int)*numLoops;
  int szFl = sizeof(float);
  int szPt = sizeof(SiftPoint);
  // 用于存放所有loop中用于计算各自H矩阵的siftpoint匹配，每次计算H矩阵需要4对匹配点。d_coord是保存匹配的两点的x和y坐标，d_randPts保存随机选择的四对点的序号
  safeCall(cudaMalloc((void **)&d_coord, 4*sizeof(float)*numPtsUp));
  safeCall(cudaMalloc((void **)&d_randPts, randSize));
  // 每次迭代计算的H矩阵都要用单独的内存来保存吗？是的，最后比较哪个的内点数最多
  ...
  // 将每个点的匹配score和ambiguities从device复制到host
  ...
  if (numValid>=8) {
    ...
    // 匹配点矩阵是umPtsUp*4（在内存中是按照列优先存储成一个数组）。前2列是当前点的x和y坐标，后2列是匹配点的x和y坐标。
    // 每一列超过numPts的部分点用什么填充？前面在分配内存时似乎没有对其初始化呀
    ...
    // 对于计算得到的H，计算所有原始匹配点集d_coord中符合这个H的点的数量（注意，不是计算validPts中的，因为validPts的选取条件其实很严格，会错失很多正确的匹配），保存在d_randPts的每个loop的第一个元素中（一列为一个loop的四个元素）
...
double MatchSiftData(SiftData &data1, SiftData &data2, float *sort_pt_ambi, int *sort_pt_id, float *dev_sort_pt)
{
  TimerGPU timer(0);
...
// Combined version with no global memory requirement using global locks
  ...
  {
    // 上面的FindMaxCorr操作中只会改变每个siftpoint的其中五个量
    // 从score开始的5个float表示的是该点的匹配结果，(score, ambiguity, match, match_xpos, match_ypos)，其中match表示所匹配的data2中的点的序号
    float *h_ptr = &data1.h_data[0].score;
    float *d_ptr = &data1.d_data[0].score;
    // 将每个点的这5个match结果复制到host中
    // 当使用cudaMemcpyDeviceToHost时，应该是自动block此处线程的吧？
    safeCall(cudaMemcpy2D(h_ptr, sizeof(SiftPoint), d_ptr, sizeof(SiftPoint), 5*sizeof(float), data1.numPts, cudaMemcpyDeviceToHost));
  }
  
  // 基于maxScore对所有的匹配进行排序，方便在CPU上根据ambiguity来均匀地选取匹配
  if(sort_pt_id != nullptr)
  {
    float *d_ptr = &data1.d_data[0].ambiguity;
    // cudaStream_t cpy_stream;
    // deviceTodevice的复制会阻塞host线程吗？如果不会，这里要如何阻塞默认流？难道要用event？
    safeCall(cudaMemcpy2D(dev_sort_pt, sizeof(float)*2, d_ptr, sizeof(SiftPoint), 1*sizeof(float), data1.numPts, cudaMemcpyDeviceToDevice));
    int *h_ptr_id = &sort_pt_id[0];
    // 这里是hostTodevice，是会阻塞host的，同样是用默认流，因此这可以保证上面的deviceTodevice已完成？
    safeCall(cudaMemcpy2D((void*)(dev_sort_pt+1), sizeof(float)*2, h_ptr_id, sizeof(int), 1*sizeof(float), data1.numPts, cudaMemcpyHostToDevice));

    GPU_CHECK(cudaDeviceSetLimit(cudaLimitDevRuntimeSyncDepth, MAX_DEPTH_2));
    int left = 0;
    int set_right = numPts1 - 1;
    float *data_ptr = dev_sort_pt;
    // 快速排序
    simple_quicksort<<<1, 1>>>(data_ptr, left, set_right, 0);
    
    safeCall(cudaMemcpy2D((void*)(h_ptr_id), sizeof(int)*1, (void*)(dev_sort_pt+1), sizeof(float)*2, 1*sizeof(float), data1.numPts, cudaMemcpyDeviceToHost));

    float *h_ptr_ambi = &sort_pt_ambi[0];
    safeCall(cudaMemcpy2D((void*)(h_ptr_ambi), sizeof(float)*1, (void*)(dev_sort_pt), sizeof(float)*2, 1*sizeof(float), data1.numPts, cudaMemcpyDeviceToHost));
  }
