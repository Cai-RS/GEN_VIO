...
float *AllocSiftTempMemory(int width, int height, int numOctaves, bool scaleUp)
{
  ...
  size_t pitch;
  size += sizeTmp;
  // 为一个每行为4096 bytes(即32个128,这个128应该是每个点的特征向量？32是一行的点数？）的2D array分配连续内存
  // 由于要求原图的长宽都要被32整除，且下采样后w会被上调至大于它的最小的128整数倍，则最小层的h至少为1，则原始图大小至少为128*32=4096，全部大小层至少为128*（31+32）
...
// 在各金字塔层中（从最小分辨率到最大）依次检测sift点，最终结果都是在原始层图像上
void ExtractSift(SiftData &siftData, CudaImage &img, int numOctaves, double initBlur, float thresh, float lowestScale, bool scaleUp, float *tempMemory, const cudaStream_t stream_) 
{
  TimerGPU timer(0);
  // for what?
  ...
  // 列数为128的整数倍方便传输？另外，每个block或者Kernel应该只负责h*128的区域？
  int p = iAlignUp(w, 128);
  ...
  // 默认sizeTmp所属的变量保存在前面的内存部分
  float *memorySub = memoryTmp + sizeTmp;

  CudaImage lowImg;
  // 分配原始图像的内存用于保存低通滤波后的图像
  ...
  if (!scaleUp) 
  {
    ...
    // 只复制检测点数这个值到host。那么检测点的信息存放在哪里？何时复制到host中？
  ...
  else 
  {
    ...
    // 只复制检测点数这个值到host
    safeCall(cudaMemcpy(&siftData.numPts, &d_PointCounterAddr[2*numOctaves], sizeof(int), cudaMemcpyDeviceToHost)); 
    siftData.actual_numPts = siftData.numPts;
    siftData.numPts = (siftData.numPts<siftData.maxPts ? siftData.numPts : siftData.maxPts);
...
void FreeSiftData(SiftData &data)
{
  ...
  data.d_data = NULL;
  if (data.malloc_host && data.h_data!=NULL)
    free(data.h_data);
#endif
  data.numPts = 0;
  data.maxPts = 0;
