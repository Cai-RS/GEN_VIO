...
namespace StereoDepth{
    ...
    struct InputPadder{
        // number of pad in top, bottom, left, right
        int Padder[4];

        int h_raw, w_raw;
        string pad_mode = "constant";
        
        void pad(const Mat &raw_img, Mat &padded_img, const string mode = "constant");
    ...
    public:
        ...
        // explicit关键字表示此函数的参数禁止隐式转换，但是仅用于单形参的函数
        DPStereo(const DPstereoConfig &stereoCGI_config, cudaStream_t stream_common);

        // 传入共用的stream用于多个模型的同步推理
        DPStereo(const DPstereoConfig &stereoCGI_config, cudaStream_t stream_common, 
                 cudaStream_t stream_cpy = nullptr);

        ~DPStereo();

        bool build();

        bool infer(cudaEvent_t stop_infer_depth = nullptr, cudaEvent_t stop_cpy_input_depth = nullptr);
        ...
        bool preprocess_input(const Mat &image_left, const Mat &image_right, cudaEvent_t stop_cpy_input_depth = nullptr);

        bool postprocess_output(Mat &unpadded_depth_map, cudaEvent_t stop_infer_depth, cudaEvent_t stop_post_proc = nullptr);

        float* get_binding_output_device();

    private:
        DPstereoConfig DPstereo_config_;
        ...
        // for some Stereo-depth model, such as DP-Stereo
...
typedef std::shared_ptr<StereoDepth::DPStereo> DPStereoPtr;
